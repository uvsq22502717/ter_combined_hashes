...
fn compress(engine: EngineType, iv: &[u8], m: u64, target_bits: u32) -> (u64, Vec<u8>) {
    ...
    match engine {
        ...
        _ => {
            // Custom Feistel-like or Lightweight hash simulation
            let state = u64::from_le_bytes(iv[0..8].try_into().unwrap()) & MASK_48;
...
fn attack_joux(eng: EngineType, cmb: ComboType, bits: u32) -> BenchResult {
    ...
    // Phase 1: Build Multicollisions on Pipe 1
    ...
    // Phase 2: Search for Collision in Pipe 2 using the Multicollision set
...
fn main() -> std::io::Result<()> {
    ...
    log_and_print(&mut log_file, &format!("Target Search Bits: {}\n", bits));

    // SUMMARY TABLE
    log_and_print(&mut log_file, "=== SUMMARY TABLE VIEW ===");
    let header = format!("{:<18} | {:<17} | {:>10} | {:>10} | {:>8}", 
                         "Algorithm", "Combination", "Dir Iter", "Joux Iter", "Ratio");
    log_and_print(&mut log_file, &header);
    log_and_print(&mut log_file, &"-".repeat(75));
    ...
    println!("\n[INFO] Results with Robust Interaction appended to {}", log_filename);
    Ok(())
}